...
export function EventRow({ topic, isRead, onToggleRead }: PrimitiveProps) {
  ...
  return (
    <article
      id={topic.slug}
      className={`py-4 first:pt-2 transition-opacity ${
        isRead ? "opacity-75" : ""
      }`}
    >
      <div className="p-3.5 rounded-xl bg-stone-50 dark:bg-stone-900/40 border border-stone-200/90 dark:border-stone-800 space-y-2.5">
        {/* Top bar with Icon + Category Pill + Controls */}
        ...
        {topic.mustMemorizeFacts && topic.mustMemorizeFacts.length > 0 && (
          <ul className="space-y-1 text-sm font-serif leading-relaxed text-[var(--text-primary)] pl-0.5">
            {topic.mustMemorizeFacts.map((fact, fIdx) => (
              <li key={fIdx} className="flex items-start gap-2">
                <span className="text-stone-400 dark:text-stone-600 font-bold select-none">•</span>
