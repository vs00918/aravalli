...
import { CheckCircle2, Circle, Trophy, BarChart3 } from "lucide-react";
...
export function RankingTable({ topic, isRead, onToggleRead }: PrimitiveProps) {
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 text-xs font-mono text-[var(--text-subtle)] select-none">
        <div className="flex items-center gap-1.5">
          <Trophy className="w-3.5 h-3.5 text-yellow-700 dark:text-yellow-400" />
          <span className="font-semibold text-[11px] uppercase tracking-wider text-yellow-950 dark:text-yellow-300">
      ...
      {/* Facts in Quiet List */}
      <div className="border-l-2 border-yellow-500/70 dark:border-yellow-400/60 pl-3.5 py-1 space-y-2 text-sm text-[var(--text-primary)] font-serif leading-relaxed">
        {facts.map((fact, idx) => (
          <div key={idx} className="flex items-start gap-2">
            <span className="text-yellow-700 dark:text-yellow-400 font-bold mt-1 text-xs select-none">•</span>
