...
function runPostDeployQaTests() {
  ...
  test('Full 12-Month Master Archive Structure (Jan–Dec 2026)', () => {
    ...
    // Check August 2026, January 2026, and February 2026 indexed sets
  ...
  test('Zero Duplicate Canonical Entities Invariant', () => {
    ...
    assert.strictEqual(slugs.size, 216, 'Must have exactly 216 unique canonical topics');
  });
  ...
  test('Category Partitioning & Grouping Invariant (All 216 Topics Accounted)', () => {
    ...
    assert.strictEqual(totalPartitioned, 216, 'All 216 monthly indexed topics must be strictly accounted');
  });
  ...
  test('Priority Density Integrity (P1, P2, P3 Content Fidelity)', () => {
    ...
    assert.strictEqual(p1s.length, 15, 'Must have exactly 15 P1 topics (7 in Aug + 4 in Jan + 4 in Feb)');
    assert.strictEqual(p2s.length, 94, 'Must have exactly 94 P2 topics (31 in Aug + 34 in Jan + 29 in Feb)');
    assert.strictEqual(p3s.length, 107, 'Must have exactly 107 P3 topics (29 in Aug + 35 in Jan + 43 in Feb)');
