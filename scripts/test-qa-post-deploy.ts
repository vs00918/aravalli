...
function runPostDeployQaTests() {
  ...
  test('Full 12-Month Master Archive Structure (Jan–Dec 2026)', () => {
    ...
    // Check August 2026, January 2026, February 2026, and March 2026 indexed sets
  ...
  test('Zero Duplicate Canonical Entities Invariant', () => {
    ...
    assert.strictEqual(slugs.size, 296, 'Must have exactly 296 unique canonical topics');
  });
  ...
  test('Category Partitioning & Grouping Invariant (All 296 Topics Accounted)', () => {
    ...
    assert.strictEqual(totalPartitioned, 296, 'All 296 monthly indexed topics must be strictly accounted');
  });
  ...
  test('Priority Density Integrity (P1, P2, P3 Content Fidelity)', () => {
    ...
    assert.strictEqual(p1s.length, 19, 'Must have exactly 19 P1 topics (7 in Aug + 4 in Jan + 4 in Feb + 4 in Mar)');
    assert.strictEqual(p2s.length, 123, 'Must have exactly 123 P2 topics (31 in Aug + 34 in Jan + 29 in Feb + 29 in Mar)');
    assert.strictEqual(p3s.length, 154, 'Must have exactly 154 P3 topics (29 in Aug + 35 in Jan + 43 in Feb + 47 in Mar)');
